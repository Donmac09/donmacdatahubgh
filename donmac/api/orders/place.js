...
//  3. Dispatches to GHData for MTN, Telecel, AirtelTigo Premium, AirtelTigo Big Time
//  4. MTN Mashup Data & MTN Mashup Minutes+Data are MANUAL delivery
//  5. All orders start as 'pending' and update via GHData status sync
...
// ============================================================
// MANUAL DELIVERY GROUPS - NEVER sent to GHData
// ============================================================
const MANUAL_DELIVERY_GROUPS = new Set(['mtn_mashup', 'mtn_mashup_min'])

// GHData accepts the network key in several casings/spellings
    ...
    try {
      ...
      if (res.ok && json?.success) {
        ...
        return {
          ...
          ghdataStatus: json.data?.status || 'processing',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      // ============================================================
      // Check if this is a MANUAL delivery group (MTN Mashup only)
      // ============================================================
      ...
      // ============================================================
      // Create order with status 'pending' for ALL orders
      // ============================================================
      ...
      if (!effectiveIsManual) {
        try {
          ...
          // ============================================================
          // Update order with GHData reference
          // Keep status as 'pending' - will be updated via status sync
          // ============================================================
          await supabaseAdmin
            .from('orders')
            .update({
              ...
              ghdata_initial_status: result.ghdataStatus || 'processing',
              // status stays 'pending' - will be updated via status check
          ...
          // Keep status as 'pending' even if GHData fails
          // Admin will see it needs manual attention
          await supabaseAdmin
            .from('orders')
            .update({
              ghdata_status: 'failed',
              ghdata_error: ghErr.message,
          ...
          for (const admin of admins || []) {
            await supabaseAdmin.from('notifications').insert({
              ...
              message: `Order ${ref} (${item.network} ${item.dataLabel}) failed to auto-deliver: ${ghErr.message}`,
              type: 'order',
            })
          }
        }
    ...
    return res.status(201).json({ 
      success: true, 
      orders: placedOrders,
      message: 'Orders placed successfully. Status will update automatically.'
    })
