...
import { Card, Btn, Input, Textarea, Modal } from '../components/ui'
...
export default function Profile() {
  ...
  async function handleUpdateProfile() {
    setSaving(true)
    try {
      const updated = await updateProfile(profile.id, { name: form.name, phone: form.phone })
  ...
  async function handleSaveWebhook() {
    try {
      await updateProfile(profile.id, { webhook_url: webhookUrl })
      updateProfileLocal({ webhook_url: webhookUrl })
  ...
  return (
    <div className="max-w-3xl space-y-6 animate-fade-in">
      ...
      <Card className="p-6">
        <div className="flex items-center gap-5 mb-6">
          ...
          <div>
            ...
            <span className="inline-block mt-1 px-3 py-0.5 bg-indigo-100 text-indigo-700 text-xs font-bold rounded-full capitalize">{profile?.role}</span>
          </div>
        </div>

        <div className="grid sm:grid-cols-2 gap-4">
          <Input label="Full Name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} icon="👤" />
          <Input label="Phone Number" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} icon="📞" />
        </div>
      ...
      <Card className="p-6">
        ...
        <p className="text-sm text-gray-500 mb-4">Use this token to authenticate API requests to Donmac Data Hub.</p>
        <div className="flex gap-2">
          ...
          <Btn onClick={() => { navigator.clipboard?.writeText(profile?.api_token); toast.success('Copied!') }} variant="secondary" size="sm">Copy</Btn>
        </div>
