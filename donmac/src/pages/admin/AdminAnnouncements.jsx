...
// Announcement color styles (matching your admin panel)
const ANN_STYLES = {
...
export default function Dashboard({ setPage }) {
  ...
  useEffect(() => {
    loadConfig()
    loadRecentOrders()
    // Load ONLY active announcements
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* 1. HERO BANNER - Greeting, Date, Time, Balance */}
      ...
      {/* 2. ANNOUNCEMENT - Now under the hero banner */}
      {announcement && (
        <div className={`flex items-center gap-3 px-4 sm:px-6 py-3 rounded-xl border shadow-sm ${ANN_STYLES[announcement.type] || ANN_STYLES.info}`}>
          <span className="text-xl flex-shrink-0">{ANN_ICONS[announcement.type] || '📢'}</span>
