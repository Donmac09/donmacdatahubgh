...
export default function AdminAnnouncements() {
  ...
  const [form, setForm] = useState({ title: '', message: '', type: 'info', active: false })
  ...
  async function toggleActive(id, val) {
    ...
    setItems(prev => prev.map(a => a.id === id ? { ...a, active: val } : a))
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Btn onClick={() => setShowCreate(true)} size="sm">+ New Announcement</Btn>
      </div>
          ...
          <Table headers={['Title', 'Message', 'Type', 'Active', 'Date', 'Actions']}>
            {items.map(a => (
              <tr key={a.id} className="hover:bg-gray-50">
                ...
                <Td>
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${TYPE_STYLES[a.type]}`}>{a.type}</span>
                </Td>
                <Td>
                  <Toggle checked={a.active} onChange={v => toggleActive(a.id, v)} size="sm" />
                </Td>
      ...
      {showCreate && (
        <Modal title="📢 New Announcement" onClose={() => setShowCreate(false)} size="sm">
          <div className="space-y-4">
            <Input label="Title" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="e.g. System Maintenance" />
            <Textarea label="Message" value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} placeholder="Enter announcement text..." />
            <div>
            ...
            <Toggle checked={form.active} onChange={v => setForm(p => ({ ...p, active: v }))} label="Show immediately to all users" />
            <Btn onClick={handleCreate} loading={saving} className="w-full" size="lg">Create Announcement</Btn>
          </div>
        </Modal>
      )}
    </div>
